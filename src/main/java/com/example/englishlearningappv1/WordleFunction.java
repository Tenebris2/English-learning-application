...
public class WordleFunction {
    ...
    private final Map<Label, String> keyboardLabelStyles = new HashMap<>();
        ...
        for (int i = 1; i <= MAX_COLUMN; i++) {
            ...
            if (keyboardLabel != null) {
                if (keyboardLabelStyles.containsKey(keyboardLabel)) {
                    // If keyboardLabel is already in the Map and its styleClass is "correct" or "wrong,"
                    // keep that status from keyboardLabelStyles.
                    String currentStyle = keyboardLabelStyles.get(keyboardLabel);
                    if (!currentStyle.equals("keyboardPresentColor")) {
                        styleClass = currentStyle;
                    } else {
                        if (styleClass.equals("keyboardCorrectColor")) {
                            styleClass = "keyboardCorrectColor";
                    ...
                    // If it's not in the Map, set the new styleClass and save it in the Map.
                    keyboardLabelStyles.put(keyboardLabel, styleClass);
                }
            }
