...
import static com.example.demo2.MainApplication.dictionaryWords;
import static com.example.demo2.MainApplication.winningWords;
...
public class MainFunction {
    ...
    private Map<Label, String> keyboardLabelStyles = new HashMap<>();
    ...
    private void updateKeyboardColors(GridPane gridPane, GridPane keyboardRow1, GridPane keyboardRow2, GridPane keyboardRow3) {
        ...
        for (int i = 1; i <= MAX_COLUMN; i++) {
            ...
            if (keyboardLabel != null) {
                if (keyboardLabelStyles.containsKey(keyboardLabel)) {
                    // Nếu keyboardLabel đã có trong Map, giữ nguyên styleClass hiện tại
                    styleClass = keyboardLabelStyles.get(keyboardLabel);
                } else {
                    // Nếu không, thiết lập styleClass mới và lưu vào Map
                    keyboardLabelStyles.put(keyboardLabel, styleClass);
                }
    ...
    private void onEnterPressed(GridPane gridPane, GridPane keyboardRow1, GridPane keyboardRow2,
        ...
        String temp = getWordFromCurrentRow(gridPane).toLowerCase();
        ...
        if (CURRENT_ROW <= MAX_ROW) {
             if (temp.length() == 5) {
                ...
                if (guess.equals(winningWord)) {
                    ...
                    com.example.demo2.ScoreWindow.display(true, winningWord);
                } else if (isValidGuess(guess)) {
                    updateRowColors(gridPane, CURRENT_ROW);
                    updateKeyboardColors(gridPane, keyboardRow1, keyboardRow2, keyboardRow3);
                    if (CURRENT_ROW == MAX_ROW) {
                        com.example.demo2.ScoreWindow.display(false, winningWord);
                    }
                    CURRENT_ROW++;
                    CURRENT_COLUMN = 1;
                } else {
                    com.example.demo2.MainApplication.showToastNotFound();
                }
            } else {
                 com.example.demo2.MainApplication.showToastShort();
             }
        }



        if (com.example.demo2.ScoreWindow.resetGame.get()) {
            resetGame(gridPane, keyboardRow1, keyboardRow2, keyboardRow3);
            com.example.demo2.ScoreWindow.resetGame.set(false);
        }

        if (com.example.demo2.ScoreWindow.quitApplication.get())
            com.example.demo2.MainApplication.quit();
